...
class GreaterThan(i1:Expression, i2:Expression) extends BooleanExpression {
  def evaluate(env:Environment) = new BooleanValue(i1.evaluate(env).intValue > i2.evaluate(env).intValue)
